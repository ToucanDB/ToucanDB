...
class SchemaManager:
    ...
    def _load_schemas(self) -> None:
        ...
        try:
            ...
            raise StorageError(
                "load", str(self.schemas_path), f"Failed to load schemas: {e}"
            ) from e
    ...
    def validate_vector(
        ...
        if not schema:
            raise InvalidSchemaError(
                f"No schema found for collection '{collection_name}'"
            )
    ...
    def _validate_schema(self, schema: VectorSchema) -> None:
        ...
        if schema.index_type == IndexType.HNSW:
            if schema.hnsw_ef_construction < schema.hnsw_m:
                raise InvalidSchemaError("HNSW ef_construction must be >= M parameter")
    ...
    def _validate_metadata(
        self, metadata: MetadataDict, schema: dict[str, str]
    ) -> None:
        ...
        for field, expected_type in schema.items():
            if field in metadata:
                ...
                if not self._check_type(value, expected_type):
                    raise ValidationError(
                        f"metadata.{field}", value, f"Expected type {expected_type}"
                    )
    ...
    def migrate_schema(
        ...
        if not current_version:
            raise InvalidSchemaError(
                f"No existing schema for collection '{collection_name}'"
            )
    ...
    def _validate_migration(
        self, old_schema: VectorSchema, new_schema: VectorSchema
    ) -> None:
    ...
    def export_schemas(self, output_path: Path) -> None:
        ...
        try:
            ...
            raise StorageError(
                "write", str(output_path), f"Failed to export schemas: {e}"
            ) from e
    ...
    def import_schemas(self, input_path: Path, overwrite: bool = False) -> list[str]:
        ...
        try:
            ...
            raise StorageError(
                "read", str(input_path), f"Failed to import schemas: {e}"
            ) from e
